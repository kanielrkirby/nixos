...
let
  bin = pkgs.writeShellScriptBin "sops-to-pass" ''
    ...
    txt=$(sops -a /home/${username}/.config/sops/age/keys.txt -d ${config.sops.secrets."primary".path})
    ...
    process_yaml "" "$txt" 0
  '';
in { environment.systemPackages = [ bin ]; }
